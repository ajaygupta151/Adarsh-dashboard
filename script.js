...
const METRIC_COLOR_MAP = {
  ...
  'Attendance': {
    main: '#0d9488',
    subs: { 'DAS': '#14b8a6', 'Inactivity': '#f43f5e' }
  },
  'EMI Collection': {
    main: '#ea580c',
    subs: { '1st EMI (Sep-Jun)': '#f97316', '2nd EMI': '#fb923c', '4th EMI': '#fdba74' }
  },
  'Test Performance and Attendance': {
    main: '#9333ea',
    subs: { 'Result': '#a855f7', 'Attendance': '#c084fc' }
  }
};
...
function cacheDOM() {
  ...
  DOM.metricChartsGrid = document.getElementById('metricChartsGrid');
...
function tryLoadSequential(urls, idx) {
  if (idx >= urls.length) {
    onLoadError(new Error('Could not reach data source. Check internet or Google Sheet publication settings.'));
    return;
  }
...
function buildDashboardData(dataRows) {
  ...
  if (rawRows.length === 0) throw new Error('No valid data rows found.');
...
/* ═══════════════════════════════════════════════════════════════
   DYNAMIC METRIC & SUB-METRIC DRILL-DOWN CHARTS
   ═══════════════════════════════════════════════════════════════ */

function renderMetricSubmetricCharts(f) {
  const container = DOM.metricChartsGrid;
  if (!container) return;

  // Filter metrics based on dropdown selection
  const selectedMetric = f.metric;
  const activeMetrics = selectedMetric === 'All' 
    ? METRIC_ORDER 
    : METRIC_ORDER.filter(m => m === selectedMetric);

  // Prepare container markup
  container.innerHTML = activeMetrics.map((metricName, idx) => {
    const chartId = 'chartMetric_' + idx;
    return `
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 md:p-5 flex flex-col">
        <div class="flex items-center justify-between mb-3 border-b border-slate-100 pb-2">
          <div class="flex items-center gap-2">
            <span class="w-2.5 h-2.5 rounded-full" style="background:${(METRIC_COLOR_MAP[metricName] || {}).main || '#6366f1'}"></span>
            <h3 class="font-bold text-sm text-slate-800">${metricName}</h3>
          </div>
          <span class="text-[10px] text-slate-400 bg-slate-50 px-2 py-0.5 rounded-full">Metric &amp; Sub-Metrics</span>
        </div>
  ...
  // Filter base rows for region, zone, and center
  ...
  // Generate charts for each active metric
  activeMetrics.forEach((metricName, idx) => {
    const chartId = 'chartMetric_' + idx;
    const colors = METRIC_COLOR_MAP[metricName] || { main: '#6366f1', subs: {} };
    
    // Find all submetrics belonging to this metric
    const submetrics = SM_LIST.filter(sm => sm.metric === metricName);

    const datasets = [];

    // 1. Dataset for Parent Metric Overall Ach %
    const parentByDate = {};
    dates.forEach(d => { parentByDate[d] = { sum: 0, count: 0 }; });
    
    filteredRows.forEach(r => {
      if (r.metric !== metricName) return;
      const b = parentByDate[r.date];
      if (b && r.metricAchPct != null) {
        b.sum += r.metricAchPct;
        b.count++;
      }
    });

    const parentValues = dates.map(d => {
      const b = parentByDate[d];
      return b.count > 0 ? Math.round((b.sum / b.count) * 100) / 100 : null;
    });

    datasets.push({
      label: `Overall ${metricName} (%)`,
      data: parentValues,
    ...
    // 2. Datasets for each Sub-Metric
    submetrics.forEach(sm => {
      const subByDate = {};
      dates.forEach(d => { subByDate[d] = { sum: 0, count: 0 }; });

      filteredRows.forEach(r => {
        if (r.metric !== metricName || r.subMetric !== sm.sub) return;
        const b = subByDate[r.date];
        if (b && r.metricAchPct != null) {
          b.sum += r.metricAchPct;
          b.count++;
        }
      ...
      const subValues = dates.map(d => {
        const b = subByDate[d];
        return b.count > 0 ? Math.round((b.sum / b.count) * 100) / 100 : null;
      });

      const subColor = colors.subs[sm.sub] || '#94a3b8';
    ...
    upsertChart('metricChart_' + idx, chartId, {
      type: 'line',
